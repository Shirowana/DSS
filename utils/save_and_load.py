# NOTE:
# This file is a mirror of the DSS-customized PEFT utility used at runtime.
# The actual runtime location in our remote/PEFT fork should be:
#   /data/home/7250091/date/quest20260313/peft/src/peft/utils/save_and_load.py
# Keep that PEFT copy and this repository mirror in sync.
#
...
def _is_dss_config(config) -> bool:
    return getattr(config, "peft_type", None) == PeftType.DSS


def _normalize_module_name(name: str) -> str:
    if name.startswith("_fsdp_wrapped_module."):
        return name.removeprefix("_fsdp_wrapped_module.")
    return name


def _collect_dss_sparse_state_dict(model, adapter_name: str) -> dict[str, torch.Tensor]:
    sparse_state_dict = {}
    for name, module in model.named_modules():
        export_fn = getattr(module, "export_sparse_checkpoint", None)
...
def _match_dss_sparse_key(key: str, adapter_name: str) -> tuple[str, str] | None:
    suffix_map = (
        (f".coefficient_indices.{adapter_name}", "coefficient_indices"),
        (".coefficient_indices", "coefficient_indices"),
        (f".coefficient.{adapter_name}", "coefficient"),
        (".coefficient", "coefficient"),
    )
    for suffix, field_name in suffix_map:
        if key.endswith(suffix):
            return key[: -len(suffix)], field_name
    return None


def _restore_dss_sparse_state_dict(
    model,
    ...
    for module_name, payload in grouped_state.items():
        if "coefficient" not in payload or "coefficient_indices" not in payload:
            raise ValueError(
                f"Incomplete DSS sparse checkpoint for module {module_name!r}: "
                f"found keys {sorted(payload.keys())}."
            )
        sub_module = model.get_submodule(module_name)
        restore_fn = getattr(sub_module, "restore_sparse_checkpoint", None)
        if not callable(restore_fn):
            raise ValueError(f"Module {module_name!r} does not support DSS sparse checkpoint restore.")
        restore_fn(adapter_name, payload["coefficient"], payload["coefficient_indices"])
...
def get_peft_model_state_dict(
    ...
    elif _is_dss_config(config):
        to_return = _collect_dss_sparse_state_dict(model, adapter_name)
...
def set_peft_model_state_dict(
    ...
    if config.is_prompt_learning or config.peft_type == PeftType.ADAPTION_PROMPT:
        ...
        peft_model_state_dict = _restore_dss_sparse_state_dict(model, state_dict, adapter_name)

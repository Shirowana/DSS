...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create or update a compact math experiment record JSON.")
...
def git_code_version(project_root: Path) -> str:
    branch = subprocess.check_output(["git", "-C", str(project_root), "rev-parse", "--abbrev-ref", "HEAD"], text=True).strip()
...
def parse_training_log(log_file: Path) -> dict:
    ...
    train_summary_re = re.compile(r"\{'train_runtime': [^}]*'train_loss': (?P<train_loss>[-+0-9.eE]+), 'epoch':")
...
def parse_trainer_state(output_dir: Path) -> dict:
    ...
    state = load_json(checkpoints[-1] / "trainer_state.json") if checkpoints else {}
    ...
    return {
        ...
        "final_checkpoint": checkpoints[-1].name if checkpoints else "",
    }
...
def update_eval_record(record: dict, args: argparse.Namespace) -> None:
    summary = load_json(Path(args.eval_output_dir) / "summary.json")
...
def main() -> None:
    args = parse_args()
    record_path = Path(args.output_dir) / "record.json"
